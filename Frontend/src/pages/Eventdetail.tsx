...
export default function EventDetails() {
    ...
    async function handleSave() {
        ...
        try {
            ...
            const res = await api<{ id?: number; event_id?: number; eventName?: string; status?: string }>(
                "/events",
                { method: "POST", body: JSON.stringify(payload) }
            )

            // รองรับทั้ง res.id และ res.event_id
            const eventId = (res.event_id ?? res.id)
            if (!eventId) {
                throw new Error("ไม่พบ event_id หรือ id ในผลลัพธ์ของ backend")
            }

            // Save ลง localStorage ไว้ใช้ต่อใน Ticket
            const cached = {
                id: eventId,
                ...payload,
